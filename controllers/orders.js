...
exports.ordersSortSearchAndFilters = async (req, res) => {
    try {
      ...
      if (sorting == 1) {
          allOrders = await orderModel.find().sort({ createdAt: 1 }).populate('userId').populate('prodId');
      ...
      if (searchText && searchText != "" && searchText.length > 0) {
        allOrders = allOrders.filter((order) => {
          return order.prodId.productDetails.Name == searchText || order.userId.name.firstName == searchText;
        });
      }

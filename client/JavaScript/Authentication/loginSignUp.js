...
form.addEventListener("click", async (event) => {
  ...
  if (userMail.value === "admin123" && userPassword.value === "Admin@123") {
    ...
    userMail.value === "E101" &&
    userPassword.value === "Angela@1234"
  ) {
  } else if (
    !userMail.value.match(employeeFormat) ||
    userMail.value.includes("@")
  ) {
    if (userMail.value == "" || userMail.value == null) {
      ...
      document.querySelector("#user-mail").classList.contains("error")
    ) {
    ...
    if (
      ...
      !userPassword.value.match(lowerCaseLetters) ||
      !userPassword.value.match(upperCaseLetters) ||
      !userPassword.value.match(numbers) ||
      ...
      document
        .querySelector(".contact-form #user-password")
        .classList.add("error");
    }
    ...
    if (confirmPassword != null && confirmPasswordError != null) {
      if (userPassword != null && userPassword.value != confirmPassword.value) {
        ...
        document
          .querySelector(".contact-form #user-confirm")
          .classList.add("error");
        // document.getElementById("password-label").style = 'transform: translateY(-30rem)';
      } else if (
        document.querySelector("#user-confirm").classList.contains("error")
      ) {
        document
          .querySelector(".contact-form #user-confirm")
    ...
    if (passwordErrorMessages.length > 0) {
      ...
      document.querySelector("#user-password").classList.contains("error")
    ) {
